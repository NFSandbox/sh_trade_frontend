...
export function ResponsiveLayout({ children }: ResponsiveLayoutProps) {
  ...
  return (
    <>
      <FlexDiv className="w-full flex-none">
        {isHeaderVisible == true && <CusHeader></CusHeader>}
      </FlexDiv>
      <FlexDiv expand className={classNames("flex-col overflow-y-auto")}>
        {children}
      </FlexDiv>
    </>
  );
...
function CusHeader() {
  ...
  return (
    <Header content={<HeaderContent />}>
      <FlexDiv className={classNames("flex-row gap-x-2", "items-center")}>
        {/* Website Icon Title */}
        <Link href={"/"}>
          <FlexDiv
...
function HoverMenuButton({
  ...
  href,
  ...args
}: { href?: string; children: ReactNode } & ButtonProps) {
  let retBtn = (
    <Button
      key={href ?? undefined}
      variant="text"
      ...
      {...args}
    >
      <FlexDiv className="w-full justify-start">{children}</FlexDiv>
    </Button>
  );
...
function UserAvatar() {
  ...
  if (userInfo) {
    ...
    return (
      <Popover
        ...
        arrow={false}
      >
        <button className={classNames(userInfoIsLoading ? "opacity-50" : "")}>
          <Avatar size={"large"} gap={0}>
  ...
  return (
    <Link
      href="/auth"
      className={classNames(userInfoIsLoading ? "opacity-50" : "")}
    >
      <p>登录 / 注册</p>
    </Link>
  );
